...
impl fmt::Display for InvalidDeclarationReason {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            InvalidDeclarationReason::MissingVersion => write!(f, "missing version"),
            InvalidDeclarationReason::InvalidVersion { version } => write!(f, "version is not valid: {}", version),
            InvalidDeclarationReason::InvalidStandalone { standalone } => {
                write!(f, "standalone flag is not valid: {}", standalone)
            }
            InvalidDeclarationReason::UnexpectedContent { content } => write!(f, "unexpected content: {}", content),
        }
    }
}
...
impl ParseError {
        ...
        S: fmt::Display,
    {
        ParseError::UnexpectedEof {
            expected: expected.map(display_iter),
        }
    }
        ...
        S: fmt::Display,
    {
        ParseError::UnexpectedToken {
            ...
            expected: display_iter(expected),
        }
    }
...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            ParseError::UnexpectedDeclaration => write!(f, "Unexpected XML declaration"),
            ParseError::UnexpectedDoctype => write!(f, "Unexpected document type definition"),
            ParseError::UnclosedAttributeValue => write!(f, "Unclosed attribute value"),
            ParseError::InvalidAttributeName { name } => write!(f, "Attribute name is not valid"),
            ParseError::InvalidDeclaration(reason) => write!(f, "XML document declaration is not valid: {}", reason),
        }
    }
}
