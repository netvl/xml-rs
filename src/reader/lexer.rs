...
impl Lexer {
    ...
    pub fn next_token<B: Read>(&mut self, b: &mut B) -> Result {
        ...
        match self.st {
            ...
            | State::CommentClosing(ClosingSubstate::Second) => Err(self.error("Unexpected end of stream")),
...
mod tests {
    ...
    fn simple_lexer_test() {
        let (mut lex, mut buf) = make_lex_and_buf(r#"<a p='q'> x<b z="y">d	</b></a><p/> <?nm ?> <!-- a c --> &nbsp;"#);
