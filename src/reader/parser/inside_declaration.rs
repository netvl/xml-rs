    ...
    pub fn inside_declaration(&mut self, t: Token, s: DeclarationSubstate) -> Option<Result> {
        ...
        match s {
            DeclarationSubstate::BeforeVersion => match t {
                ...
                Token::Character('v') => self.into_state_continue(State::InsideDeclaration(
            ...
            DeclarationSubstate::InsideVersion => self.read_qualified_name(
                ...
                |this, token, name| match &name.local_name[..] {
                    "ersion" if name.namespace.is_none() => this.into_state_continue(
            ...
            DeclarationSubstate::AfterVersion => match t {
                ...
                Token::EqualsSign => self.into_state_continue(State::InsideDeclaration(
                    DeclarationSubstate::InsideVersionValue,
                )),
                _ => unexpected_token!(t),
            },
            ...
            DeclarationSubstate::AfterVersionValue => match t {
                ...
                Token::Character('e') => self.into_state_continue(State::InsideDeclaration(
                    DeclarationSubstate::InsideEncoding,
                )),
                Token::Character('s') => self.into_state_continue(State::InsideDeclaration(
            ...
            DeclarationSubstate::InsideEncoding => self.read_qualified_name(
                ...
                QualifiedNameTarget::AttributeNameTarget,
                |this, token, name| match &name.local_name[..] {
                    "ncoding" if name.namespace.is_none() => this.into_state_continue(
                        State::InsideDeclaration(if token == Token::EqualsSign {
            ...
            DeclarationSubstate::InsideStandaloneDecl => self.read_qualified_name(
                ...
                QualifiedNameTarget::AttributeNameTarget,
                |this, token, name| match &name.local_name[..] {
                    "tandalone" if name.namespace.is_none() => this.into_state_continue(
                        State::InsideDeclaration(if token == Token::EqualsSign {
