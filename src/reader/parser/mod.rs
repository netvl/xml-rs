...
use common::{self, is_name_char, is_name_start_char, Position, TextPosition, XmlVersion};
use name::OwnedName;
...
impl PullParser {
        ...
        F: Fn(&mut PullParser, Token, OwnedName) -> Option<Result>,
    {
        ...
        match t {
            ...
            Token::EqualsSign if target == QualifiedNameTarget::AttributeNameTarget => {
                invoke_callback(self, t)
            }

            Token::EmptyTagEnd if target == QualifiedNameTarget::OpeningTagNameTarget => {
                invoke_callback(self, t)
            }
