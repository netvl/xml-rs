    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self.0 {
            Ok(ref e) => match *e {
                ...
                xml::Event::Comment(ref data) => write!(f, r#"Comment("{}")"#, data.escape_debug()),
                xml::Event::CData(ref data) => write!(f, r#"CData("{}")"#, data.escape_debug()),
                xml::Event::Text(ref data) => write!(f, r#"Characters("{}")"#, data.escape_debug()),
                xml::Event::Whitespace(ref data) => write!(f, r#"Whitespace("{}")"#, data.escape_debug()),
            },
            Err(ref e) => e.fmt(f),
        }
    }
